...
SSwitchButton::SSwitchButton(QWidget* parent)
	...
	m_radius(8.0),
	m_nMargin(3),
	m_checkedThumbColor(Qt::white),
	m_checkBackground(QColor("#409EFF")),
	m_unCheckThumbColor(249, 249, 249),
	m_unCheckBackground(Qt::gray),
	m_disabledColor(Qt::green)
{
	// 鼠标滑过光标形状 - 手型
	setCursor(Qt::PointingHandCursor);

	// 连接信号槽
	connect(&m_timer, &QTimer::timeout, this, &SSwitchButton::onTimeout);
}
...
void SSwitchButton::paintEvent(QPaintEvent* event)
{
	...
	if (isEnabled()) { // 可用状态
		if (m_bState) { // 打开状态
			...
			thumbColor = m_checkedThumbColor;
			dOpacity = 0.600;
		}
		else { //关闭状态
			background = m_unCheckBackground;
			thumbColor = m_unCheckThumbColor;
			dOpacity = 0.800;
		}
	}
	else {  // 不可用状态
